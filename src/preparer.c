...
void generate_static_np_expressions(union expression **s, region ct_reg, struct expansion_context *ectx) {
	switch((*s)->base.type) {
		case begin: {
			union expression **exprr;
			{foreachaddress(exprr, (*s)->begin.expressions) {
				generate_static_np_expressions(exprr, ct_reg, ectx);
			}}
			break;
		} case with: {
			generate_static_np_expressions(&(*s)->with.expression, ct_reg, ectx);
			break;
		} case _if: {
			generate_static_np_expressions(&(*s)->_if.condition, ct_reg, ectx);
			generate_static_np_expressions(&(*s)->_if.consequent, ct_reg, ectx);
			generate_static_np_expressions(&(*s)->_if.alternate, ct_reg, ectx);
			break;
		} case function: {
			generate_static_np_expressions(&(*s)->function.expression, ct_reg, ectx);
			break;
		} case continuation: {
			generate_static_np_expressions(&(*s)->continuation.expression, ct_reg, ectx);
			break;
		} case invoke: case jump: case storage: {
			if((*s)->base.type != storage) {
				generate_static_np_expressions(&(*s)->invoke.reference, ct_reg, ectx);
			}
			union expression **arg;
			{foreachaddress(arg, (*s)->invoke.arguments) {
				generate_static_np_expressions(arg, ct_reg, ectx);
			}}
			break;
		} case non_primitive: {
			if(!(*s)->non_primitive.dynamic_context) {
				generate_static_np_expressions(&(*s)->non_primitive.reference, ct_reg, ectx);
				...
				*macro_cache = NULL;
				...
				union expression *wth = make_with(make_reference(NULL, ct_reg),
				...
				prepend(invoke_return_with_ref_arg, &args_ct, ct_reg);
				...
				put(macro_invocation, with.expression, make_jump(make_invoke1(make_literal((unsigned long) _get_, ct_reg),
					callee_cont_arg, ct_reg), args_ct, ct_reg));
...
void (*dynamic_np_expansion(list (*expander)(list, region), list argument, struct expansion_context *ectx, list st_binds, list dyn_ref_names, list indirections, void (*(*macro_cache))(void *)))(void *) {
	...
	list cont_params = nil;
	char *ref_name;
	{foreach(ref_name, reverse(dyn_ref_names, ct_reg)) {
		prepend(make_reference(ref_name, ct_reg), &cont_params, ct_reg);
	}}
	...
	union expression *guest_cont_param = make_reference(NULL, ct_reg);
	prepend(guest_cont_param, &cont_params, ct_reg);
	union expression *guest_cont_arg = make_reference(NULL, ct_reg);
	...
	union expression *cont = make_continuation(make_reference(NULL, ct_reg), cont_params,
		make_jump1(make_invoke1(make_literal((unsigned long) _get_, ct_reg), guest_cont_arg, ct_reg),
...
void generate_dynamic_np_expressions(union expression **s, region ct_reg, struct expansion_context *ectx) {
	switch((*s)->base.type) {
		case begin: {
			union expression **exprr;
			{foreachaddress(exprr, (*s)->begin.expressions) {
				generate_dynamic_np_expressions(exprr, ct_reg, ectx);
			}}
			break;
		} case with: {
			generate_dynamic_np_expressions(&(*s)->with.expression, ct_reg, ectx);
			break;
		} case _if: {
			generate_dynamic_np_expressions(&(*s)->_if.condition, ct_reg, ectx);
			generate_dynamic_np_expressions(&(*s)->_if.consequent, ct_reg, ectx);
			generate_dynamic_np_expressions(&(*s)->_if.alternate, ct_reg, ectx);
			break;
		} case function: {
			generate_dynamic_np_expressions(&(*s)->function.expression, ct_reg, ectx);
			break;
		} case continuation: {
			generate_dynamic_np_expressions(&(*s)->continuation.expression, ct_reg, ectx);
			break;
		} case invoke: case jump: case storage: {
			if((*s)->base.type != storage) {
				generate_dynamic_np_expressions(&(*s)->invoke.reference, ct_reg, ectx);
			}
			union expression **arg;
			{foreachaddress(arg, (*s)->invoke.arguments) {
				generate_dynamic_np_expressions(arg, ct_reg, ectx);
			}}
			break;
		} case non_primitive: {
			if((*s)->non_primitive.dynamic_context) {
				generate_dynamic_np_expressions(&(*s)->non_primitive.reference, ct_reg, ectx);
