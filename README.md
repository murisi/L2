...
This expression is implemented by emitting an instruction to `mov` an immediate value into a memory location determined by the surrounding expression.
...
This expression is implemented by the emission of an instruction to `lea` of some data into a memory location determined by the surrounding expression.
