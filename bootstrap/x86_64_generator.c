...
void layout_frames(union expression *n, list *binding_augs, region r) {
  switch(n->base.type) {
    case function: {
...
void make_load(struct binding_aug *bndg, unsigned long offset, union expression *dest_reg, union expression *scratch_reg, list *c, region r) {
  switch(bndg->type) {
    case frame_relative_storage: {
      prepend(make_asm3(MOVQ_MDB_TO_REG, make_literal(bndg->offset + offset, NULL, NULL, r), make_asm0(RBP, r), dest_reg, r), c, r);
      break;
    } case absolute_storage: {
      prepend(make_asm2(MOVQ_IMM_TO_REG, make_asm2(LNKR_ADD_OFF_TO_REF, use_binding(bndg, r), make_literal(offset, NULL, NULL, r), r),
        scratch_reg, r), c, r);
      prepend(make_asm3(MOVQ_MDB_TO_REG, make_literal(0, NULL, NULL, r), scratch_reg, dest_reg, r), c, r);
      break;
    } case top_relative_storage: {
      prepend(make_asm3(MOVQ_MDB_TO_REG, make_literal(bndg->offset + offset, NULL, NULL, r), make_asm0(RSP, r), dest_reg, r), c, r);
      break;
    }
  }
}

void make_store(union expression *src_reg, struct binding_aug *bndg, unsigned long offset, union expression *scratch_reg, list *c, region r) {
  switch(bndg->type) {
    case frame_relative_storage: {
      prepend(make_asm3(MOVQ_REG_TO_MDB, src_reg, make_literal(bndg->offset + offset, NULL, NULL, r), make_asm0(RBP, r), r), c, r);
      break;
    } case absolute_storage: {
      prepend(make_asm2(MOVQ_IMM_TO_REG, make_asm2(LNKR_ADD_OFF_TO_REF, use_binding(bndg, r), make_literal(offset, NULL, NULL, r), r),
        scratch_reg, r), c, r);
      prepend(make_asm3(MOVQ_REG_TO_MDB, src_reg, make_literal(0, NULL, NULL, r), scratch_reg, r), c, r);
      break;
    } case top_relative_storage: {
      prepend(make_asm3(MOVQ_REG_TO_MDB, src_reg, make_literal(bndg->offset + offset, NULL, NULL, r), make_asm0(RSP, r), r), c, r);
      break;
    }
  }
}
...
void make_load_address(struct binding_aug *bndg, union expression *dest_reg, list *c, region r) {
  switch(bndg->type) {
    case frame_relative_storage: {
      prepend(make_asm3(LEAQ_MDB_TO_REG, make_literal(bndg->offset, NULL, NULL, r), make_asm0(RBP, r), dest_reg, r), c, r);
      break;
    } case absolute_storage: {
      prepend(make_asm2(MOVQ_IMM_TO_REG, use_binding(bndg, r), dest_reg, r), c, r);
      break;
    } case top_relative_storage: {
      prepend(make_asm3(LEAQ_MDB_TO_REG, make_literal(bndg->offset, NULL, NULL, r), make_asm0(RSP, r), dest_reg, r), c, r);
      break;
    }
  }
}
...
void sgenerate_symbols(union expression *n, list *c, region r) {
...
void cond_push_relative_storage(union expression *n, list *c, region r) {
  if(n->jump.temp_storage_bndg->type == top_relative_storage) {
    prepend(make_asm2(SUBQ_IMM_TO_REG, make_literal(length(n->jump.arguments) * WORD_SIZE, NULL, NULL, r), make_asm0(RSP, r), r), c, r);
  }
}

void cond_pop_relative_storage(union expression *n, list *c, region r) {
  if(n->jump.temp_storage_bndg->type == top_relative_storage) {
    prepend(make_asm2(ADDQ_IMM_TO_REG, make_literal(length(n->jump.arguments) * WORD_SIZE, NULL, NULL, r), make_asm0(RSP, r), r), c, r);
  }
}

void generate_args_to_buffer(union expression *n, list *c, region r) {
  int offset = 0;
  union expression *t;
...
void generate_buffer_to_dest(union expression *n, union expression *dest_reg, int offset, list *c, region r) {
  int tmp_offset = 0;
  union expression *t;
  foreach(t, n->jump.arguments) {
    make_load(n->jump.temp_storage_bndg, tmp_offset, make_asm0(RAX, r), make_asm0(R10, r), c, r);
    prepend(make_asm3(MOVQ_REG_TO_MDB, make_asm0(RAX, r), make_literal(offset, NULL, NULL, r), dest_reg, r), c, r);
    offset += WORD_SIZE;
...
void sgenerate_jumps(union expression *n, list *c, region r) {
  if(n->jump.short_circuit) {
    ...
    prepend(make_asm3(MOVQ_MDB_TO_REG, make_literal(CONT_RBX, NULL, NULL, r), make_asm0(R11, r), make_asm0(RBX, r), r), c, r);
    prepend(make_asm3(MOVQ_MDB_TO_REG, make_literal(CONT_R12, NULL, NULL, r), make_asm0(R11, r), make_asm0(R12, r), r), c, r);
    prepend(make_asm3(MOVQ_MDB_TO_REG, make_literal(CONT_R13, NULL, NULL, r), make_asm0(R11, r), make_asm0(R13, r), r), c, r);
    prepend(make_asm3(MOVQ_MDB_TO_REG, make_literal(CONT_R14, NULL, NULL, r), make_asm0(R11, r), make_asm0(R14, r), r), c, r);
    prepend(make_asm3(MOVQ_MDB_TO_REG, make_literal(CONT_R15, NULL, NULL, r), make_asm0(R11, r), make_asm0(R15, r), r), c, r);
    prepend(make_asm3(MOVQ_MDB_TO_REG, make_literal(CONT_CIR, NULL, NULL, r), make_asm0(R11, r), make_asm0(R10, r), r), c, r);
    prepend(make_asm3(MOVQ_MDB_TO_REG, make_literal(CONT_RBP, NULL, NULL, r), make_asm0(R11, r), make_asm0(RBP, r), r), c, r);
    prepend(make_asm1(JMP_TO_REG, make_asm0(R10, r), r), c, r);
  }
}

void sgenerate_literals(union expression *n, list *c, region r) {
  prepend(make_asm2(MOVQ_IMM_TO_REG, make_literal(n->literal.value, NULL, NULL, r), make_asm0(RAX, r), r), c, r);
}
...
void sgenerate_functions(union expression *n, list *c, region r) {
  ...
  prepend(make_asm2(SUBQ_IMM_TO_REG, make_literal(-get_current_offset(n), NULL, NULL, r), make_asm0(RSP, r), r), c, r);
  ...
  prepend(make_asm1(POPQ_REG, make_asm0(R11, r), r), c, r);
  prepend(make_asm2(ADDQ_IMM_TO_REG, make_literal(6*WORD_SIZE, NULL, NULL, r), make_asm0(RSP, r), r), c, r);
  prepend(make_asm1(PUSHQ_REG, make_asm0(R11, r), r), c, r);
...
void sgenerate_invokes(union expression *n, list *c, region r) {
  ...
  prepend(make_asm2(MOVQ_IMM_TO_REG, make_literal(0, NULL, NULL, r), make_asm0(RAX, r), r), c, r);
  
  prepend(make_asm1(CALL_REG, make_asm0(R11, r), r), c, r);
  
  if(length(n->invoke.arguments) > 6) {
    prepend(make_asm2(ADDQ_IMM_TO_REG, make_literal(WORD_SIZE * (length(n->invoke.arguments) - 6), NULL, NULL, r),
      make_asm0(RSP, r), r), c, r);
  }
}

void sgenerate_constrains(union expression *n, list *c, region r) {
...
list generate_program(list exprs, region r) {
  list c = nil;
