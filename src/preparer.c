...
void *static_np_expansion(list (*expander)(list, region, list), list argument, struct expansion_context *ectx, list st_binds,
	...
	union expression *macro_cont = make_continuation(make_reference(NULL, ct_reg), lst(guest_cont_param, nil, ct_reg),
		make_jump1(make_invoke1(make_literal((unsigned long) _get_, ct_reg), guest_cont_arg, ct_reg),
			build_expression(expansion, ectx->rt_reg, ectx->handler), ct_reg), ct_reg);
	refer_reference(guest_cont_arg, guest_cont_param);
	
	Object *obj = load_expressions(make_program(lst(make_storage(make_reference(NULL, ct_reg), lst(macro_cont, nil, ct_reg), ct_reg),
		nil, ct_reg), ct_reg), ectx, st_binds, ct_reg);
	list is = concat(ectx->ext_binds, st_binds, ct_reg);
	...
	mutate_symbols(obj, is);
	//Initialize our annonymous storage with our continuation
	((void (*)()) segment(obj, ".text"))();
	//There is only one immutable symbol, our annonymous storage, and it is housing our continuation.
	*macro_cache = *((void **) ((object_symbol *) immutable_symbols(obj, ct_reg)->fst)->address);
	*macro_sexpr_cache = expansion;
	destroy_region(ct_reg);
	return *macro_cache;
}
